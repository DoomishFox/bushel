...
import { ZeroWidthSpace } from "#prosemirror/Characters.ts";

/**
 * @public
 */
export class InclusiveExclusiveMarkInputRule extends InputRule {
  ...
  private markHandler(
    ...
    if (!InclusiveExclusiveMarkInputRule.markApplies(state.doc, range, this.markType)) {
      return null;
    }
    ...
    const tr = state.tr.replaceWith(
      ...
      this.markType.schema.text(ZeroWidthSpace + match[1] + ZeroWidthSpace),
    );
