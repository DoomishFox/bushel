...
import type { Attrs, MarkType, Node as ProseMirrorNode } from "prosemirror-model";
import {
  type EditorState,
  SelectionRange,
  TextSelection,
 ...
 * @public
 */
export class LinkInputRule extends InputRule {
  ...
  private markHandler(
    ...
    if (!LinkInputRule.markApplies(state.doc, range, this.markType)) {
      return null;
    }
    ...
    for (const markType of newMarks) {
      tr.addMark(
        ...
        tr.mapping.map(end),
        markType.create({ href: match[2], title: match[3] ?? undefined }),
      );
    }
